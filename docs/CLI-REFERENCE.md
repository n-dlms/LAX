...
**Why it exists:** status is rich; sometimes you want one glanceable number, e.g. watching it fall during `lax crash`.
...
### `lax rebalance [--local]`
Combined move: withdraw excess collateral and repay debt in one step, improving capital efficiency while restoring HF. A tidy-up command for
