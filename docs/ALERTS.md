...
Set one environment variable (`LAX_ALERT_WEBHOOK`) and every fire path in LAX, the autopilot daemon, the CLI engage path, and the demo command, pushes a
...
- The alert request times out after 5 seconds and failures are logged to stderr. They never touch the mitigation pipeline.
