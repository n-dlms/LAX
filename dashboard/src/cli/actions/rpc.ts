...
export async function handleKeeper(_ctx: CommandContext): Promise<CommandResult> {
  const apiKey = import.meta.env.VITE_KEEPERHUB_API_KEY as string | undefined;
  if (!apiKey) return { output: "KeeperHub: no API key configured", error: "Missing VITE_KEEPERHUB_API_KEY" };
  return {
